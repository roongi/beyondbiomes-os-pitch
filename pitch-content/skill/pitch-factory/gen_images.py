...
PROMPTS = {
 ...
 "map_hero": (
    "Antique treasure-map composition on warm aged parchment with fine sepia line-art, BUT a clear GREEN (#3E7C4F leaf) "
    "and ORANGE (#C7521E) brand colour scheme for ALL accents, icons, routes and emblems - NO blue anywhere, minimal "
    "gold. Keep the central layer, the connections/agents and the principles in the SAME green-and-orange palette so it "
    "reads as ONE connected system (do NOT put the agents in a separate colour). ONE compass rose (top-left), a couple "
    "of small ships in the margins. PROFESSIONAL, symmetrical, self-contained, fills the frame. CRITICAL: every label "
    "and icon EXACTLY ONCE - NO duplicates. KEEP ALL TEXT EXACT and legible. "
    "Top title banner, elegant serif: 'BEYOND BIOMES - AS NATURE INTENDED'. Subtitle ribbon: 'the AI-native platform "
    "for regenerative agriculture - a multidisciplinary community system'. "
    "EXACT CENTER: an ornate hub labelled 'THE AGENTIC LAYER' wrapped by a CIRCULAR LOOP of arrows showing the "
    "deep-tech lifecycle in order: 'PLAN -> IMPLEMENT -> REVIEW -> VERIFY -> SHIP -> LEARN'. "
    "FOUR connections (green + orange), ONE per quadrant: top-left 'HUMANS <-> HUMANS - farmer, producer, scientist'; "
    "top-right 'HUMANS <-> AGENTS - a living app for each'; bottom-left 'AGENTS <-> ENVIRONMENT - we read the field'; "
    "bottom-right 'AGENTS <-> BACTERIA - we design consortia'. "
    "A WIDE foundation band across the bottom titled 'NATURE'S RULE = OUR DEEP-TECH RULE', with FOUR cells, each "
    "pairing a nature principle to its deep-tech keyword with a SHORT sentence: "
    "'PROOFREADER = REVIEW - a separate checker re-reads the work'; "
    "'IMMUNITY = VERIFY - check against a reference nothing can fake'; "
    "'QUORUM SENSING = SHIP - act only when the shared signal crosses the threshold'; "
    "'STIGMERGY = LEARN - the trail is the ledger, feedback heals the ground'. "
    "Top-right frontier mark: 'BEYOND - next verticals'. Bottom ribbon: 'build the eyes before the muscles - we go far "
    "together'. Leave a clean EMPTY CIRCLE in the bottom-RIGHT corner for a logo (do NOT draw any logo or emblem there). "
    "Warm parchment + green and orange accents only, NO blue, professional, balanced, legible, no duplicates, 16:9."),
}
